...
def test_import_export_import_form(admin_client):
    ...
    assert (
        '<label for="id_import_file" class="block font-semibold text-important whitespace-nowrap">'
...
def test_import_export_export_form(admin_client):
    ...
    assert response.status_code == HTTPStatus.OK
    assert (
        '<label for="id_format" class="block font-semibold text-important whitespace-nowrap">'
        in response.content.decode()
    )
